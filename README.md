...
3. Create and config the `.env` file   
   ...
   ENV=production
   ```
Only use `ENV` variable equals to `production` when you intend to deploy the commands to all the servers, otherwise leave it blank. If you are editing and testing new features with the bot **DO NOT** asign the variable to `production`.  
